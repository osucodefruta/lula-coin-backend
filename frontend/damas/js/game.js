/**
 * Lógica local do jogo de damas para validação de movimentos.
 * Este objeto não interage com a API ou com a cena 3D.
 */
const LocalGameLogic = {
    isValidMove: function(board, from, to, player, isKing) {
        ...
        if (!isKing) {
        ...
        } else { // Lógica da Dama (King)
            if (Math.abs(dr) !== Math.abs(dc)) return { valid: false }; // Movimento não diagonal
            ...
            for (let i = 1; i < Math.abs(dr); i++) {
                ...
                if (pieceOnPath !== 0) {
                    ...
                    else { return { valid: false }; } // Bloqueado por peça amiga
                }
            }
            if (capturedCount > 1) return { valid: false }; // Não pode pular mais de uma peça
    ...
    canPieceCapture: function(board, fromRow, fromCol) {
        ...
        if (!isKing) {
            for (const [dr, dc] of directions) {
                ...
                if (destRow >= 0 && destRow < 8 && destCol >= 0 && destCol < 8 && opponentRow >= 0 && opponentRow < 8 && opponentCol >= 0 && opponentCol < 8) {
...
/**
 * Lógica principal do jogo, incluindo Three.js, UI e comunicação com a API.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ELEMENTOS DA UI ---
    // CORREÇÃO 1: Adicionada a declaração da variável ausente
    ...
    const apiRequest = async (endpoint, method = 'GET', body = null) => {
        ...
        if (!token) { 
            // CORREÇÃO 2: Roteamento para caminho absoluto
    ...
    // --- CRIAÇÃO DE OBJETOS 3D E TEXTURAS ---
    ...
    function handleSquareClick(clickedObject) {
        ...
        if (moveInfo.valid) {
            ...
            if (currentPiece === 1 && row === 0) currentPiece = 3; // Promove para Dama
            if (currentPiece === 2 && row === 7) currentPiece = 4; // Promove para Dama
    ...
    // --- LOOP DE ANIMAÇÃO E INICIALIZAÇÃO ---
    ...
    if (!onlineState.gameId) {
        ...
        createEnvironment();
        // CORREÇÃO 2: Roteamento para caminho absoluto
        const returnToLobby = () => { window.location.href = '/'; };
