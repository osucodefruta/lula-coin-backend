// Aguarda o carregamento completo do DOM para iniciar o jogo
document.addEventListener('DOMContentLoaded', () => {
  ...
  const apiRequest = async (endpoint, method = 'GET', body = null) => {
      ...
      if (!token) { 
          ...
          // CORREÇÃO 1: Roteamento para caminho absoluto
  ...
  // --- FUNÇÕES DE GERAÇÃO DE TEXTURA PROCEDURAL ---
  ...
  // CORREÇÃO 2: Função 'comprar' refatorada para não ser global
  function comprar(item) { 
    const itemUpper = item.toUpperCase(); 
    if (itemUpper === 'TERRENO') { 
        mostrarModalConfirmacao(`Deseja comprar Terreno por ${recursos.precoTerreno} LCO?`, comprarTerrenoComLCO); 
        return; 
    } 
    const c = ITENS_LOJA[itemUpper]; 
    if (!c) return; 
  ...
  function animate() {
    ...
    if (animais.length > 1) { aplicarFisicaAnimais(); }
    raycaster.setFromCamera(mouse, camera);
    ...
    if (!isTooltipVisible) { tooltip.style.display = 'none'; }
  ...
  // CORREÇÃO 2: Adiciona event listeners para os botões da loja
  document.querySelectorAll('.botao-loja').forEach(button => {
  ...
  // CORREÇÃO 2: Adiciona event listener para o botão de fechar modal de sementes
  document.getElementById('fechar-modal-sementes-btn').addEventListener('click', fecharModalSementes);
  ...
  window.addEventListener('beforeunload', () => {
      if (farmState && Object.keys(farmState).length > 0) {
          // Usando sendBeacon para garantir que a requisição seja enviada mesmo com a página fechando
